...
def test1():
    ...
    request_body = {
        ...
        "negative_prompt": "nsfw, (worst quality, low quality:1.4), (blurry:1.2), (greyscale, monochrome:1.1), 3D face, cropped, lowres, text, jpeg artifacts, signature, watermark, username, blurry, artist name, trademark, watermark, title, multiple view, reference sheet, plump, fat, muscular female, strabismus, out of frame, ugly, extra limbs, (bad anatomy), gross proportions, (malformed limbs), ((missing arms)), ((missing legs)), (((extra arms))), (((extra legs))), mutated hands, (fused fingers), (too many fingers), (((long neck)))",
        "seed": 47552436,
        "steps": 30,
        "cfg_scale": 8,
    ...
    response = requests.get("http://localhost:8000/txt2img", json=request_body)
...
def test2():
    request_body = {
        "img_path": "images/image_1686315044_0.png"
    }

    response = requests.get("http://127.0.0.1:8000/upscale", json=request_body, timeout=240)
    output = response.json()
